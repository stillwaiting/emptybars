...
const SecondPage = ({ pageContext, data }) => {
  return (
    ...
    >
      <SEO
        title={
          `${data.composition.edges[0].node.fields.content} (${data.composer.edges[0].node.fields.content}), ` +
      ...
      <div>
        ...
        <script
          ...
          dangerouslySetInnerHTML={{
            __html: JSON.stringify(pageContext.sections, null, 3),
          }}
        ></script>
      </div>
      <Link to={data.sheetMusicUrl.edges[0].node.fields.content}>
      ...
      <Helmet>
        ...
        {data.playerJs.urls.map((url, idx) => (
          <script src={url} defer={true} key={"playerJs" + idx} />
        ))}
      </Helmet>
    </Layout>
  )
...
export const query = graphql`
  query sectionsTemplateQuery(
    $composerNamePath: String
    $compositionNamePath: String
    $performerNamePath: String
    $sheetMusicUrlPath: String
  ) {
    playerCss {
      urls
    }
    ...
    composition: allFile(
      filter: { relativePath: { eq: $compositionNamePath } }
    ) {
      edges {
        node {
          fields {
    ...
    sheetMusicUrl: allFile(
      filter: { relativePath: { eq: $sheetMusicUrlPath } }
    ) {
      edges {
        node {
          fields {
