...
function addPerformer(indexedData, node) {
  ...
  indexedData[composerSlug].compositions[compositionSlug].performers[
    performerSlug
  ] = {
    name: node.fields.content,
  }
...
const IndexPage = ({ data }) => {
  ...
  const composerSlugsSorted = data.composers.nodes.map(
  ...
  const renderPerformer = (
    ...
    return (
      <li key={compositionSlug + "_" + performerSlug}>
        <Link to={`${composerSlug}/${compositionSlug}/${performerSlug}`}>
          {compositionName}, performed by{" "}
          {
            indexedData[composerSlug].compositions[compositionSlug].performers[
              performerSlug
            ].name
  ...
  const renderComposer = composerSlug => {
    const compositionsSorted = Object.keys(
      indexedData[composerSlug].compositions
    )
    ...
    return (
      <li key={composerSlug}>
        ...
        <ul>
          {compositionsSorted.map(compositionSlug =>
            renderComposition(composerSlug, compositionSlug)
          )}
        </ul>
      </li>
    )
  ...
  return (
    <Layout composition="EmptyBars.org">
      <SEO title="Home" />
      <ul>{composerSlugsSorted.map(renderComposer)}</ul>
    </Layout>
  )
}
...
export const query = graphql`
  query MyQuery {
    ...
    compositions: allFile(
      filter: { fields: { type: { eq: "compositionName" } } }
    ) {
      nodes {
        relativeDirectory
        fields {
