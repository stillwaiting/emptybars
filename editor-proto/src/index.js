import React from 'react';
import ReactDOM from 'react-dom';

import EditorLoader from './EditorLoader';
import ImageAreaSelector from "./ImageAreaSelector";
import PagesList from './PagesList';

if (document.getElementById('playerPlaceholder')) {

// Valse Romantique
  InitEditor('playerPlaceholder',

      {
          "fragments": [
              {
                  "startSec": 0.9,
                  "endSec": 4.7,
                  "pages": {
                      "1602829105573": true
                  },
                  "pageAreas": {
                      "1602829105573": [
                          {
                              "x": 200,
                              "y": 377,
                              "width": 1295,
                              "height": 401
                          },
                          {
                              "x": 10,
                              "y": 850,
                              "width": 190,
                              "height": 377
                          }
                      ]
                  }
              },
              {
                  "startSec": 4.3,
                  "endSec": 11.6,
                  "pages": {
                      "1602829105573": true
                  },
                  "pageAreas": {
                      "1602829105573": [
                          {
                              "x": 0,
                              "y": 822,
                              "width": 1496,
                              "height": 421
                          },
                          {
                              "x": 17,
                              "y": 1275,
                              "width": 537,
                              "height": 459
                          }
                      ]
                  }
              },
              {
                  "startSec": 11.7,
                  "endSec": 17.8,
                  "pages": {
                      "1602829105573": true
                  },
                  "pageAreas": {
                      "1602829105573": [
                          {
                              "x": 550,
                              "y": 1302,
                              "width": 945,
                              "height": 414
                          },
                          {
                              "x": 10,
                              "y": 1798,
                              "width": 1003,
                              "height": 374
                          }
                      ]
                  }
              },
              {
                  "startSec": 17.7,
                  "endSec": 20.7,
                  "pages": {
                      "1602829105573": true,
                      "1602829114898": true
                  },
                  "pageAreas": {
                      "1602829105573": [
                          {
                              "x": 996,
                              "y": 1795,
                              "width": 605,
                              "height": 401
                          }
                      ],
                      "1602829114898": [
                          {
                              "x": 13,
                              "y": 244,
                              "width": 343,
                              "height": 329
                          }
                      ]
                  }
              },
              {
                  "startSec": 20.7,
                  "endSec": 26.7,
                  "pages": {
                      "1602829114898": true
                  },
                  "pageAreas": {
                      "1602829114898": [
                          {
                              "x": 353,
                              "y": 210,
                              "width": 1156,
                              "height": 370
                          },
                          {
                              "x": 40,
                              "y": 649,
                              "width": 992,
                              "height": 319
                          }
                      ]
                  }
              },
              {
                  "startSec": 26.6,
                  "endSec": 35.3,
                  "pages": {
                      "1602829114898": true
                  },
                  "pageAreas": {
                      "1602829114898": [
                          {
                              "x": 1023,
                              "y": 639,
                              "width": 520,
                              "height": 326
                          },
                          {
                              "x": 10,
                              "y": 1064,
                              "width": 1570,
                              "height": 323
                          },
                          {
                              "x": 17,
                              "y": 1417,
                              "width": 581,
                              "height": 363
                          }
                      ]
                  }
              },
              {
                  "startSec": 35.3,
                  "endSec": 39.6,
                  "pages": {
                      "1602829114898": true
                  },
                  "pageAreas": {
                      "1602829114898": [
                          {
                              "x": 605,
                              "y": 1441,
                              "width": 928,
                              "height": 319
                          },
                          {
                              "x": 20,
                              "y": 1832,
                              "width": 159,
                              "height": 360
                          }
                      ]
                  }
              },
              {
                  "startSec": 39.1,
                  "endSec": 58.6,
                  "pages": {
                      "1602829114898": true,
                      "1602829121156": true
                  },
                  "pageAreas": {
                      "1602829114898": [
                          {
                              "x": 20,
                              "y": 1819,
                              "width": 1513,
                              "height": 377
                          }
                      ],
                      "1602829121156": [
                          {
                              "x": 3,
                              "y": 207,
                              "width": 1547,
                              "height": 377
                          },
                          {
                              "x": 27,
                              "y": 605,
                              "width": 1499,
                              "height": 370
                          }
                      ]
                  }
              },
              {
                  "startSec": 58.6,
                  "endSec": 70.4,
                  "pages": {
                      "1602829121156": true
                  },
                  "pageAreas": {
                      "1602829121156": [
                          {
                              "x": 30,
                              "y": 989,
                              "width": 1455,
                              "height": 367
                          }
                      ]
                  }
              },
              {
                  "startSec": 70.4,
                  "endSec": 88.4,
                  "pages": {
                      "1602829121156": true
                  },
                  "pageAreas": {
                      "1602829121156": [
                          {
                              "x": 1526,
                              "y": 2186,
                              "width": -1516,
                              "height": -765
                          }
                      ]
                  }
              },
              {
                  "startSec": 88.4,
                  "endSec": 92.7,
                  "pages": {
                      "1602829125201": true
                  },
                  "pageAreas": {
                      "1602829125201": [
                          {
                              "x": 6,
                              "y": 207,
                              "width": 1196,
                              "height": 414
                          }
                      ]
                  }
              },
              {
                  "startSec": 92.4,
                  "endSec": 99.8,
                  "pages": {
                      "1602829125201": true
                  },
                  "pageAreas": {
                      "1602829125201": [
                          {
                              "x": 1098,
                              "y": 255,
                              "width": 476,
                              "height": 387
                          },
                          {
                              "x": 20,
                              "y": 652,
                              "width": 1526,
                              "height": 387
                          }
                      ]
                  }
              },
              {
                  "startSec": 99.8,
                  "endSec": 105.8,
                  "pages": {
                      "1602829125201": true
                  },
                  "pageAreas": {
                      "1602829125201": [
                          {
                              "x": 20,
                              "y": 1047,
                              "width": 1519,
                              "height": 333
                          },
                          {
                              "x": 10,
                              "y": 1428,
                              "width": 581,
                              "height": 316
                          }
                      ]
                  }
              },
              {
                  "startSec": 105.8,
                  "endSec": 109.1,
                  "pages": {
                      "1602829125201": true
                  },
                  "pageAreas": {
                      "1602829125201": [
                          {
                              "x": 598,
                              "y": 1424,
                              "width": 958,
                              "height": 336
                          }
                      ]
                  }
              },
              {
                  "startSec": 108.8,
                  "endSec": 120.3,
                  "pages": {
                      "1602829125201": true,
                      "1602829130363": true
                  },
                  "pageAreas": {
                      "1602829125201": [
                          {
                              "x": 1455,
                              "y": 1451,
                              "width": 102,
                              "height": 319
                          },
                          {
                              "x": 20,
                              "y": 1812,
                              "width": 1536,
                              "height": 367
                          }
                      ],
                      "1602829130363": [
                          {
                              "x": 10,
                              "y": 224,
                              "width": 1526,
                              "height": 353
                          },
                          {
                              "x": 27,
                              "y": 595,
                              "width": 1020,
                              "height": 357
                          }
                      ]
                  }
              },
              {
                  "startSec": 120.3,
                  "endSec": 123.6,
                  "pages": {
                      "1602829130363": true
                  },
                  "pageAreas": {
                      "1602829130363": [
                          {
                              "x": 1050,
                              "y": 595,
                              "width": 489,
                              "height": 363
                          },
                          {
                              "x": 3,
                              "y": 982,
                              "width": 564,
                              "height": 350
                          }
                      ]
                  }
              },
              {
                  "startSec": 123.6,
                  "endSec": 128.7,
                  "pages": {
                      "1602829130363": true
                  },
                  "pageAreas": {
                      "1602829130363": [
                          {
                              "x": 571,
                              "y": 975,
                              "width": 962,
                              "height": 360
                          },
                          {
                              "x": 13,
                              "y": 1356,
                              "width": 1030,
                              "height": 401
                          }
                      ]
                  }
              },
              {
                  "startSec": 128.8,
                  "endSec": 139.9,
                  "pages": {
                      "1602829130363": true
                  },
                  "pageAreas": {
                      "1602829130363": [
                          {
                              "x": 802,
                              "y": 1523,
                              "width": 0,
                              "height": 0
                          },
                          {
                              "x": 1037,
                              "y": 1377,
                              "width": 510,
                              "height": 391
                          },
                          {
                              "x": 37,
                              "y": 1754,
                              "width": 1468,
                              "height": 377
                          }
                      ]
                  }
              }
          ],
          "pages": [
              {
                  "url": "https://github.com/stillwaiting/emptybars/raw/main/debussy/childrens-corner-1-doctor/notes-images/d14f484a79853cb003ee61d42486313d-1.jpg",
                  "id": 1602829105573
              },
              {
                  "url": "https://github.com/stillwaiting/emptybars/raw/main/debussy/childrens-corner-1-doctor/notes-images/d14f484a79853cb003ee61d42486313d-2.jpg",
                  "id": 1602829114898
              },
              {
                  "url": "https://github.com/stillwaiting/emptybars/raw/main/debussy/childrens-corner-1-doctor/notes-images/d14f484a79853cb003ee61d42486313d-3.jpg",
                  "id": 1602829121156
              },
              {
                  "url": "https://github.com/stillwaiting/emptybars/raw/main/debussy/childrens-corner-1-doctor/notes-images/d14f484a79853cb003ee61d42486313d-4.jpg",
                  "id": 1602829125201
              },
              {
                  "url": "https://github.com/stillwaiting/emptybars/raw/main/debussy/childrens-corner-1-doctor/notes-images/d14f484a79853cb003ee61d42486313d-5.jpg",
                  "id": 1602829130363
              }
          ],
          "videoUrl": "https://www.youtube.com/watch?v=1k_OLrK4ZuE"
      }
  );


  function InitEditor($element, video) {
    const {fragments, pages, videoUrl, videoTitle} = video;

    ReactDOM.render(
        <React.StrictMode>
          <EditorLoader fragments={fragments} pages={pages} videoUrl={videoUrl} />
        </React.StrictMode>,
        document.getElementById($element)
    );
  }

}


if (document.getElementById('areaSelector')) {
  const areas = [
    {
      x: 10, y: 10, width: 150, height: 50
    },
    {
      x: 300, y: 200, width: 250, height: 150
    }
  ];

  ReactDOM.render(
      <React.StrictMode>
        <ImageAreaSelector
            imgUrl="https://images.sftcdn.net/images/t_app-cover-l,f_auto/p/befbcde0-9b36-11e6-95b9-00163ed833e7/260663710/the-test-fun-for-friends-screenshot.jpg"
            width={300}
            areas={areas}
            onNewAreaAdded={item => alert(JSON.stringify(item, null, 2))}
            onDeleteArea={item => alert(item)}
        />
      </React.StrictMode>,
      document.getElementById('areaSelector')
  );
}

if (document.getElementById('pagesList')) {

  ReactDOM.render(
      <React.StrictMode>
        <PagesList
            pages={[]} onPagesUpdated={() => {}}
        />
      </React.StrictMode>,
      document.getElementById('pagesList')
  );
}

